...
POSITION_METRICS = {
    "GK": {
        "metrics": {"Save %": "save_pct", "Goals Prevented": "goals_prevented_p90",
                    "Sweeper Actions": "sweeper_actions_p90", "Aerial Duels Won": "aerial_duels_won_pct",
                    "xGC /90": "xgc_p90", "Clean Sheets": "clean_sheets"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Save %","save_pct"),
    ...
    "CB": {
        "metrics": {"Def. Actions /90": "def_actions_p90", "Def. Duels Won %": "def_duels_won_pct",
                    "Aerial Won %": "aerial_duels_won_pct", "Interceptions /90": "interceptions_p90",
                    "Tackles /90": "tackles_p90", "Pass Acc %": "passes_accurate_pct",
                    "Long Pass Acc %": "long_passes_accurate_pct"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Def. Actions/90","def_actions_p90"),
                   ("Def. Duels Won%","def_duels_won_pct"),("Aerial Won%","aerial_duels_won_pct"),
                   ("Interc./90","interceptions_p90"),("Tackles/90","tackles_p90"),
                   ("Long Pass Acc%","long_passes_accurate_pct")],
        "group": "Defender",
    },
    "LB": {
        "metrics": {"Def. Actions /90": "def_actions_p90", "Def. Duels Won %": "def_duels_won_pct",
                    "Crosses /90": "crosses_p90", "Cross Acc %": "crosses_accurate_pct",
                    "Prog. Passes /90": "progressive_passes_p90", "xA /90": "xa_p90",
                    "Dribbles /90": "dribbles_p90"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Def. Actions/90","def_actions_p90"),
                   ("Def. Duels Won%","def_duels_won_pct"),("Crosses/90","crosses_p90"),
                   ("Cross Acc%","crosses_accurate_pct"),("Prog. Passes/90","progressive_passes_p90"),
                   ("xA/90","xa_p90")],
        "group": "Defender",
    },
    "RB": {
    ...
    "DMF": {
        "metrics": {"Def. Actions /90": "def_actions_p90", "Def. Duels Won %": "def_duels_won_pct",
                    "Interceptions /90": "interceptions_p90", "Pass Acc %": "passes_accurate_pct",
                    "Prog. Passes /90": "progressive_passes_p90", "Tackles /90": "tackles_p90",
                    "Aerial Won %": "aerial_duels_won_pct"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Def. Actions/90","def_actions_p90"),
                   ("Def. Duels Won%","def_duels_won_pct"),("Interc./90","interceptions_p90"),
                   ("Tackles/90","tackles_p90"),("Pass Acc%","passes_accurate_pct"),
                   ("Prog. Passes/90","progressive_passes_p90")],
        "group": "Midfielder",
    },
    "CMF": {
        "metrics": {"Prog. Passes /90": "progressive_passes_p90", "Key Passes /90": "key_passes_p90",
                    "Pass Acc %": "passes_accurate_pct", "Def. Actions /90": "def_actions_p90",
                    "xA /90": "xa_p90", "Dribbles /90": "dribbles_p90",
                    "F3 Passes /90": "final_third_passes_p90"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Prog. Passes/90","progressive_passes_p90"),
                   ("Key Passes/90","key_passes_p90"),("Pass Acc%","passes_accurate_pct"),
                   ("xA/90","xa_p90"),("Def. Actions/90","def_actions_p90"),
                   ("F3 Passes/90","final_third_passes_p90")],
        "group": "Midfielder",
    },
    "AMF": {
        "metrics": {"Key Passes /90": "key_passes_p90", "xA /90": "xa_p90",
                    "xG /90": "xg_p90", "Dribbles /90": "dribbles_p90",
                    "Prog. Passes /90": "progressive_passes_p90",
                    "Box Touches /90": "box_touches_p90", "Shots /90": "shots_p90"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Key Passes/90","key_passes_p90"),
                   ("xA/90","xa_p90"),("xG/90","xg_p90"),("Dribbles/90","dribbles_p90"),
                   ("Box Touches/90","box_touches_p90"),("Shots/90","shots_p90")],
        "group": "Midfielder",
    },
    "WF": {
        "metrics": {"Goals /90": "goals_p90", "xG /90": "xg_p90", "xA /90": "xa_p90",
                    "Dribbles /90": "dribbles_p90", "Crosses /90": "crosses_p90",
                    "Shots /90": "shots_p90", "Prog. Runs /90": "progressive_runs_p90"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Goals","goals"),
                   ("Goals/90","goals_p90"),("xG/90","xg_p90"),("xA/90","xa_p90"),
                   ("Dribbles/90","dribbles_p90"),("Crosses/90","crosses_p90")],
        "group": "Forward",
    },
    "CF": {
        "metrics": {"Goals /90": "goals_p90", "xG /90": "xg_p90", "Shots /90": "shots_p90",
                    "Shot on Tgt %": "shots_on_target_pct", "Aerial Won %": "aerial_duels_won_pct",
                    "Box Touches /90": "box_touches_p90", "Dribbles /90": "dribbles_p90"},
        "detail": [("Jogos","matches"),("Minutos","minutes"),("Goals","goals"),
...
GROUP_FALLBACK = {
    ...
    "Midfielder": "CMF", "Forward": "CF",
}


def _get_pos_key(position_str, position_group):
...
def _get_position_group(pos_group):
    return {"goalkeeper": "Goalkeeper", "defender": "Defender",
            "midfielder": "Midfielder", "forward": "Forward"}.get(str(pos_group).lower(), "Midfielder")


def _section(title):
    st.markdown(
        f'<div style="font-size:15px;font-weight:700;color:{GOLD};'
        f'margin:24px 0 12px 0;border-left:3px solid {GOLD};padding-left:10px;">'
        f'{title}</div>',
...
def _radar(player_row, group_df, pos_key, player_name):
    ...
    fig.update_layout(
        ...
        title=dict(text=f"{player_name} — {pos_key}",
...
def _percentile_bars(player_row, group_df, pos_key):
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color=colors, marker_line=dict(color=BG, width=1),
...
def render(players):
    ...
    if players.empty:
        st.warning("Sem dados de jogadores.")
        return
    ...
    with col_info:
        ...
        mins  = player_row.get("minutes", "—")
        games = player_row.get("matches", "—")
        st.markdown(
            f'<div style="background:{CARD_BG};border:1px solid {GRID};border-radius:10px;'
            f'padding:14px 20px;margin-top:4px;">'
            f'<span style="color:{GOLD};font-size:18px;font-weight:700;">{selected}</span>'
            f'<span style="color:{MUTED};font-size:13px;margin-left:12px;">{pos} · {pos_key}</span><br>'
            f'<span style="color:{MUTED};font-size:12px;">Idade: {age} &nbsp;|&nbsp; '
            f'Jogos: {games} &nbsp;|&nbsp; Minutos: {mins}</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
